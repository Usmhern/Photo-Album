/* ==========================================================================
   1. INTRO OVERLAY & TYPING SEQUENCE
   ========================================================================== */
(function () {
    const lines = [
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!overlay || !enterBtn || elements.includes(null)) {
            console.error("Missing intro overlay elements");
            return;
        }

        elements[0].classList.add('line-body');
        ...
        function openBookEffect() {
            // Play welcome sound if element exists
            const welcomeSound = document.getElementById("welcomeSound");
            if (welcomeSound) {
                welcomeSound.currentTime = 0;
                welcomeSound.play().catch(err => console.error("Audio playback error:", err));
            }
...
/* ==========================================================================
   2. MAIN GALLERY & DIRECTORY APPLICATION LOGIC
   ========================================================================== */
...
const nigeriaStatesAndLgas = {
    ...
    "Oyo": ["Afijio", "Akinyele", "Atiba", "Atisbo", "Egbeda", "Ibadan North", "Ibadan North-East", "Ibadan North-West", "Ibadan South-East", "Ibadan South-West", "Ibarapa Central", "Ibarapa East", "Ibarapa North", "Iddo", "Irepo", "Iseyin", "Itesiwaju", "Iwajowa", "Kajola", "Lagelu", "Ogbomosho North", "Ogbomosho South", "Ogo Oluwa", "Olorunsogo", "Oluyole", "Ona Ara", "Orelope", "Ori Ire", "Oyo East", "Oyo West", "Saki East", "Saki West", "Surulere"],
...
// Attach preview sound event listener once globally
document.addEventListener('DOMContentLoaded', () => {
    const previewModal = document.getElementById("preview-modal");
    const BSound = document.getElementById("BSound");

    if (previewModal && BSound) {
        previewModal.addEventListener("click", () => {
            BSound.currentTime = 0;
            BSound.play().catch(err => console.error("Preview sound error:", err));
        });
    }

    // Initialize state dropdowns & fetch live data
...
async function fetchGalleryData() {
    try {
        ...
        if (grid) grid.innerHTML = '<div class="loading-text">Failed to load memories.</div>';
    }
}
...
function handleStateChange(stateSelectId, lgaContainerId, lgaSelectId) {
    ...
    if (!stateSelect || !lgaContainer || !lgaSelect) return;
...
async function saveMemberEdit() {
    ...
    const performSubmit = async (dataPayload) => {
        try {
            ...
            console.error(err);
            alert('Error updating details. Check browser console.');
        } finally {
